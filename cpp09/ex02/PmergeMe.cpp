...
void	VectSort::initSequences(std::vector<int> &main, std::vector<int> &pend)
{
	...
	for (int i = 0; i < _order * 2; i++) //push a1 et b1 dans le main
...
void		VectSort::binaryInsert(std::vector<int> &main, std::vector<int> &pend, int elemIdx, int min, int max)
{
	...
	for (int i = 0; i < insertIdx * _order; i++)
		viMain++;
	for (int i = 0; i < elemIdx * _order; i++)
		viPend++;
	for (int i = 0; i < _order; i++)
	{
...
long	VectSort::jacobsthal(long n) {return round((pow(2, n + 1) + pow(-1, n)) / 3);}
